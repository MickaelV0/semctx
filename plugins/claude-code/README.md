...
- **Guard hook** (`hooks/`): a `PreToolUse` guard that is **inert by default** (advisory) and, when
  ...
  selection abbreviations are rejected, and a repository git hook is rejected unless the project
  declares it for the verb being gated — a declared `pre-commit` may restage, and the tree-based
  push gate is what forces a fresh verification of what it produced.
  Push requires an explicit non-delegating remote and exact `HEAD`; configured push/server options, remote helpers,
